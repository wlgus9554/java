	...
	// 3. 글등록 처리
		...
		public int write(NoticeVO vo) throws Exception {
			...
			try {
				...
				e.printStackTrace();
				// 특별한 예외는 그냥 전달한다.
				throw new Exception("예외 발생 : 공지 등록 DB처리 중 예외가 발생했습니다.");
		...
		public int update(NoticeVO vo) throws Exception {
			...
			try {
				...
				// 그 외 처리 중 나타나는 오류에 대해서 사용자가 볼 수 있는 예외로 만들어 전달한다.
				else throw new Exception("예외 발생 : 공지 수정 DB처리 중 예외가 발생했습니다.");
			} finally {
				// 7. 닫기
				DB.close(con, pstmt);
			}
		...
		public int delete(Long no) throws Exception {
			...
			try {
				...
				if(e.getMessage().indexOf("예외 발생") >= 0) throw e;
				// 그 외 처리 중 나타나는 오류에 대해서 사용자가 볼 수 있는 예외로 만들어 전달한다.
				else throw new Exception("예외 발생 : 공지 삭제 DB처리 중 예외가 발생했습니다.");
			} finally {
				// 7. 닫기
				DB.close(con, pstmt);
			}
